...
public class ColorPickerScreen extends Screen {
    ...
    private void renderColorPicker(GuiGraphics guiGraphics) {
        ...
        // 1.20.1: Tesselator hands out a single shared BufferBuilder which you begin()
        // explicitly; the 1.21 `Tesselator.begin(...)` / `addVertex(...)` fluent API
        // does not exist yet.
        ...
        for (int y = 0; y < PICKER_SIZE; y += step) {
            for (int x = 0; x < PICKER_SIZE; x += step) {
                ...
                bufferBuilder.vertex(matrix, x1, y2, 0).color(r, g, bl, 255).endVertex();
                bufferBuilder.vertex(matrix, x2, y2, 0).color(r, g, bl, 255).endVertex();
                bufferBuilder.vertex(matrix, x2, y1, 0).color(r, g, bl, 255).endVertex();
                bufferBuilder.vertex(matrix, x1, y1, 0).color(r, g, bl, 255).endVertex();
            }
        }
    ...
    private void renderHueBar(GuiGraphics guiGraphics) {
        ...
        for (int y = 0; y < PICKER_SIZE; y += step) {
            ...
            bufferBuilder.vertex(matrix, hueBarX, y2, 0).color(r, g, bl, 255).endVertex();
            bufferBuilder.vertex(matrix, hueBarX + HUE_BAR_WIDTH, y2, 0).color(r, g, bl, 255).endVertex();
            bufferBuilder.vertex(matrix, hueBarX + HUE_BAR_WIDTH, y1, 0).color(r, g, bl, 255).endVertex();
            bufferBuilder.vertex(matrix, hueBarX, y1, 0).color(r, g, bl, 255).endVertex();
        }
    ...
    public void onClose() {
        ...
        PacketHandler.CHANNEL.sendToServer(new ColorUpdatePacket(blockPos, selectedColor));
        super.onClose();
    }
    ...
    public void renderBackground(@NotNull GuiGraphics guiGraphics) {
        // Don't render default dirt/gradient background
